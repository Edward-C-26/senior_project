...
General Functions
    readRegister2949: bool readRegister2949(CommandCodeTypedef command, uint8_t address, uint16_t *data);
        - reads register specified in command from specified board
        - returns true if received PEC matches calculated PEC

Configuration Functions
    writeConfig: void writeConfig(BMSconfigStructTypedef cfg) *** [incomplete, unnecessary as of 2024 BMS]
        - configure chip/faults
        - writes configuration data to 2949
    readConfig: bool readConfig(uint8_t address, uint8_t cfg[6]); [unnecessary]
        - uses general readRegister2949 function to check current state of LTC configuration register
        - this is mostly for testing purposes
        - returns true if received PEC matches calculated PEC (same as readRegister2949)

Data Functions
    readPackCurrent: bool readPackCurrent()
    ...
    readPackVoltage: bool readPackVoltage()
        - gets values from readRegister2949
        - converts to unit value
        - stores value in variables within .h file
        - returns true if received PEC matches calculated PEC for every register read

    readPackPower: bool readPackPower()
        - gets values from readRegister2949
        - converts to unit value
        - stores value in variables within .h file
        - returns true if received PEC matches calculated PEC for every register read

    readPackCharge: bool readPackCharge()
        - gets values from readRegister2949
        - converts to unit value
        - stores value in variables within .h file
        - returns true if received PEC matches calculated PEC for every register read

    readPackEnergy: bool readPackEnergy()
        - gets values from readRegister2949
        - converts to unit value
        - stores value in variables within .h file
        - returns true if received PEC matches calculated PEC for every register read
    
    readMaxCurrent: bool readMaxCurrent()
        - gets values from readRegister2949
        - converts to unit value
        - stores value in variables within .h file
        - returns true if received PEC matches calculated PEC for every register read

    readMinCurrent: bool readMinCurrent()
        - gets values from readRegister2949
        - converts to unit value
        - stores value in variables within .h file
        - returns true if received PEC matches calculated PEC for every register read
    
    readMaxPower: bool readMaxPower()
        - gets values from readRegister2949
        - converts to unit value
        - stores value in variables within .h file
        - returns true if received PEC matches calculated PEC for every register read

    readMinPower: bool readMinPower()
        - gets values from readRegister2949
        - converts to unit value
        - stores value in variables within .h file
        - returns true if received PEC matches calculated PEC for every register read
...
// void init_PEC15_Table() // initialize Packet Error Checking Table
// {
//     uint16_t remainder;
//     for (int i = 0; i < 256; i++)
//     {
...
//uint16_t calculatePEC(uint8_t len, uint8_t *data) // calculate Packet Error Checking value
//{
...
//void wakeup_idle() { // pin PA4 (chip select pin on 2949)
//	uint32_t delay = 15;
...
bool readPackCurrent(){
    ...
    packCurrent = (int32_t)(current * 950 / 50); // current in mV
    return datavalid;
}
...
bool readPackVoltage(){ 
    ...
    packVoltage = (int32_t)(voltage * 375 * 1000); // voltage in mV
    return datavalid;
}
...
bool readPackPower(){ 
    ...
    packPower = (int32_t)(power * 58368 / 500); // power in mW
    return datavalid;
}
...
bool readMaxMinCurrent(){
    ...
    maxCurrent = (uint32_t)(maxCurr);
    minCurrent = (uint32_t)(minCurr);
    return datavalid;
}
...
bool readMaxMinPower(){
    ...
    maxPower = (uint32_t)(maxPow);
    minPower = (uint32_t)(minPow);
    return datavalid;
}
